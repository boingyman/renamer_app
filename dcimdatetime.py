...
of when it was captured. It is not very readable as the digits are only separated between the date and time of day.
...
def rename(name):
    ...
    search_pattern = re.compile(r"(?P<path>.*/)?(?P<year>\d{4})(?P<month>\d{2})(?P<day>\d{2})_(?P<hours>\d{2})" +
                                r"(?P<minutes>\d{2})(?P<seconds>\d{2})(?P<extra>.*)(?P<ext>\..*)?", re.I)
    ...
    result = s_obj.groupdict()['year'] + " " + s_obj.groupdict()['month'] + " " + s_obj.groupdict()['day'] + " - " + \
        s_obj.groupdict()['hours'] + "_" + s_obj.groupdict()['minutes'] + "_" + s_obj.groupdict()['seconds'] + \
        s_obj.groupdict()['extra']
